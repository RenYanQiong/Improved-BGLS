...
def bgls(t, y, err, plow, phigh, ofac):
	...
	return 1./f, p
# def ReadFilename():
# 	temp = open('Aster200.txt','r')
# 	FN = []
# 	for line in temp:
# 		fields = line.split()
# 		FN.append((fields[0])+'.txt')
# 	temp.close()
# 	return FN


def searchPrd(): 	#search rough results by BGLS
...
# 	Periods=np.array(Periods) # For comets or another objects
	chi_2=[]#sum of (y-yi)^2
	temp_x=0
...
# 			T_temp.append(newton(dp,ddp,x_new,Periods[0]))	
			T_temp.append(prd_newton)
	T_temp=np.array(T_temp)
	
	testtemp=p(T_temp)
# 	print T_temp,testtemp
	
# 	fig10 = plt.figure(10)	# red points are the results of f'(x)=0
# 	plt.plot(T_temp,testtemp,'or',temp,y,'*b')#
# # 	plt.title(['PredictPrd=',num2str(T),' BestPrd=',num2str(P(p)),' hours'],'fontsize',14)
# 	plt.xlabel('Periods / Hours',size = 14)
# # 	plt.ylabel('Chi_2', size = 14)
# 	plt.ylabel(''+r' '+'$ \mathrm{x}^{2}$'+'', size = 14)
...
# 		if abs(p(Temporary)-p(T_temp[i]))>(mean(T_temp))/10 and T_temp[i] not in T:
	...
	Fig_pre,y_pre,p=foldedLc(t,T_pre )
	plt.figure(num=11, figsize=(8,6))
# 	plt.title('Observed Light Curves', size = 14)
# 	plt.xlabel('time(hours)', size = 14)
# 	plt.ylabel('Magnitudes', size = 14)
	plt.plot(Fig_pre, y_pre, color = 'b', linestyle =
...
# 	x_temp=np.linspace(0,PreP[position],4000)
# 	y_temp=p(x_temp)
# 	
	fig3 = plt.figure (3)
	plt.plot(TEMP,y,'*b',TEMP,y2,'or')
#  	plt.plot(TEMP,y,'*b',x_temp,y_temp,'or')
...
# y = sum(y)/len(MAG)
# y = sum(1000000/(10**((np.array(MAG)-1)/(2.5))))/len(MAG)
err = np.array(ERR)
